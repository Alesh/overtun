...
from overtun.primitives import TargetTraffic, TrafficRule
...
async def selective_proxy(proxy_address, outlet_address):
    ...
    def traffic_selector(address: Address) -> TargetTraffic:
        return {
            Address("mail.ru", 443): TargetTraffic(rule=TrafficRule.TUNNEL),
            Address("ok.ru", 443): TargetTraffic(rule=TrafficRule.DROP),
        }.get(address, TargetTraffic(rule=TrafficRule.DIRECT))
    ...
    async def selective_proxy_context(bag: list[t.Any] | None = None):
        ...
        proxy_server = await overtun.create_server(
            proxy_address,
            make_protocol_factory(outlet_address, traffic_selector, bag=bag),
        )
        await outlet_server.start_serving()
        await proxy_server.start_serving()
        yield proxy_address
