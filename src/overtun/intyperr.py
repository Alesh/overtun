...
class TargetRegistry[Desc](t.Protocol):
    """
    Интерфейс функции или вызываемого объекта предназначенного для получения дескриптора
    целевого адреса, по его сетевому адресу. Если дескриптор не найден, возвращается `None`.
    """

    def __call__(self, target_address: Address) -> Desc | None:
        """
        Возвращает дескриптор целевого адреса.

        Args:
            target_address: Целевой сетевой адрес.

        Returns:
            Дескриптор целевого адреса если найден, иначе `None`.
        """
